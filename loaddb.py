...
from sqlalchemy import create_engine, text # create_engine function creates a connection to the database 
...
from datetime import datetime # might not be needed
...
def accounts(number_of_rows):
    ...
    for _ in range(number_of_rows):
        ...
        # users table
        id = fake.unique.numerify(text='C#########') # primary key
        name = fake.name() 
        ...
        db.execute(text("INSERT INTO users (id,name,user_type,password) VALUES (:i,:n,:t,:p)"), {"i": id,"n": name,"t": user_type,"p": password}) 
        db.commit() # saves the transaction

        # customers table
        cust_id = fake.unique.numerify(text='0###') # primary key
        cust_ssn_id = fake.unique.numerify(text='SS######') # starts with 'SS' then the rest is digits
        name = fake.name()
        address = fake.street_name()
        age = fake.random_int(18, 105)
        state = fake.state()
        city = fake.city()
        status = fake.random_element(elements=('Inactive','Active'))

        db.execute(text("INSERT INTO customers (cust_id,cust_ssn_id,name,address,age,state,city,status) VALUES (:i,:u,:n,:a,:g,:s,:c,:t)"), {"i": cust_id, "u": cust_ssn_id, "n": name, "a": address, "g": age, "s": state, "c": city, "t": status})
        db.commit()
        

        # customerlog table
        log_id = fake.unique.numerify(text='#####') # primary key
        ...
        db.execute(text("INSERT INTO customerlog (log_id,cust_id,log_message,time_stamp) VALUES (:l,:c,:m,:t)"), {"l": log_id,"c":cust_ida,"m":log_message ,"t": time_stamp})
        db.commit()

        # accounts
        acc_id = fake.unique.numerify(text='###') # primary key
        ...
        db.execute(text("INSERT INTO accounts (acc_id,acc_type,balance,cust_id,status,message,last_update) VALUES (:i,:a,:b,:c,:s,:m,:l)"), {"i": acc_id, "a": acc_type, "b": balance, "c":cust_idb, "s": status, "m": message, "l": last_update})
        db.commit()

        # transactions
        trans_id = fake.unique.numerify(text='###') # primary key
        acc_ida = acc_id # from Accounts table
        trans_message = fake.random_element(elements=('Deposit','Withdrawal'))
        amount = fake.random_int(0,10000)
        time_stamp = fake.date_time_this_decade()

        db.execute(text("INSERT INTO transactions (trans_id,acc_id,trans_message, amount, time_stamp) VALUES (:t,:a,:r,:m,:s)"), {"t": trans_id, "a": acc_ida, "r": trans_message, "m": amount, "s": time_stamp})
        db.commit()
    
if __name__ == "__main__": # executes if the script is run directly vs being imported into another script
    number_of_rows = 1
